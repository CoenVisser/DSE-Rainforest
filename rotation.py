...
start_euler = [0, beta_bumper_post, alpha_bumper_post] #There are two versions pre and post
end_euler = [0, beta_bumper_post+theta*np.cos(alpha_bumper_post), alpha_bumper_post+np.sin(alpha_bumper_post)]
...
for R_i in rotations:
    ...
    # --- LINE ---
    line_vector = R_i.apply([l_hooks_post*np.cos(np.deg2rad(alpha_hooks_post))*np.cos(np.deg2rad(beta_hooks_post)),
                            l_hooks_post*np.sin(np.deg2rad(alpha_hooks_post))*np.cos(np.deg2rad(beta_hooks_post)),
                            l_hooks_post*np.sin(np.deg2rad(beta_hooks_post))])
